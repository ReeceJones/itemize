...
async def handle_fastapi_exception(r: Request, e: BaseError) -> None:
    match e:
        ...
        case InvalidCredentialsError(msg):
            return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED, content={'detail': msg})
        ...
        case ImageNotFoundError(msg):
            return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content={'detail': msg})
        case _:
            return JSONResponse(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, content={'detail': e.msg, 'type': type(e).__name__})
